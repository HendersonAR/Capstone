...
metrics_df <- data.frame(
  Metric = c("Accuracy", "Precision", "Recall", "F1-Score", "Kappa"),
  Value = c(accuracy, precision, recall, f1_score, kappa)
)

# Use kable to create a formatted table
kable(metrics_df, col.names = c("Metric", "Value"), caption = "Model Evaluation Metrics")
```
...
conf_matrix_g <- ggplot(data = conf_df, aes(x = Reference, y = Prediction)) +
  geom_tile(aes(fill = Freq), color = "white") +
  scale_fill_gradient(low = "blue", high = "red") +
  geom_text(aes(label = Freq)) +
  theme_minimal() +
  labs(title = "LightGbm Confusion Matrix", fill = "Frequency",
...
```{r}
ggsave("lightgbm_roc.png", plot = roc_g, width = 8, height = 6, bg = "white")
ggsave("lightgbm_pr_curve.png", plot = pr_curve_g, width = 8, height = 6,
       bg = "white")
ggsave("lightgbm_conf_matrix.png", plot = conf_matrix_g, width = 8, height = 6,
       bg = "white")
ggsave("lightgbm_var_import.png", plot = variable_importance_g, width = 8, 
       height = 6, bg = "white")
```
