...
fw_transfer_sample <- transfers[sample(nrow(fw_transfers), nrow(fw_nontransfers)),]
...
```{r}
fw_trainingIndex <- createDataPartition(fw_balanced_data$transfer,
                                        p = 0.7, list = FALSE)
fw_train_data <- fw_balanced_data[fw_trainingIndex,]
fw_test_data <- fw_balanced_data[-fw_trainingIndex, ]
...
```{r}
ggsave("undersampled_fw_roc.png", plot = fw_roc_g, width = 8,
       height = 6, bg = "white")
ggsave("undersampled_fw_conf_matrix.png", plot = fw_conf_matrix_g,
...
mf_transfer_sample <- transfers[sample(nrow(mf_transfers), nrow(mf_nontransfers)),]
...
```{r}
mf_trainingIndex <- createDataPartition(mf_balanced_data$transfer,
                                        p = 0.7, list = FALSE)
mf_train_data <- mf_balanced_data[mf_trainingIndex,]
mf_test_data <- mf_balanced_data[-mf_trainingIndex, ]
...
```{r}
ggsave("undersampling_mf_roc.png", plot = mf_roc_g, width = 8,
       height = 6, bg = "white")
ggsave("undersampling_mf_conf_matrix.png", plot = mf_conf_matrix_g,
       width = 8, height = 6, bg = "white")
...
df_transfer_sample <- transfers[sample(nrow(df_transfers), nrow(df_nontransfers)),]
df_balanced_data <- rbind(df_transfer_sample, df_nontransfers)
```

```{r}
set.seed(278)
df_trainingIndex <- createDataPartition(df_balanced_data$transfer,
                                        p = 0.7, list = FALSE)
df_train_data <- df_balanced_data[df_trainingIndex,]
df_test_data <- df_balanced_data[-df_trainingIndex, ]
...
```{r}
ggsave("undersampling_df_roc.png", plot = df_roc_g, width = 8,
       height = 6, bg = "white")
ggsave("undersampling_df_conf_matrix.png", plot = df_conf_matrix_g,
       width = 8, height = 6, bg = "white")
