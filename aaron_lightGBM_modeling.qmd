...
```{r}
lgbtrain <- lgb.Dataset(data = as.matrix(train_data[, setdiff(names(train_data), "label")]), label = train_data$transfer)
lgbtest <- lgb.Dataset(data = as.matrix(test_data[, setdiff(names(test_data), "label")]), label = test_data$transfer)
```
