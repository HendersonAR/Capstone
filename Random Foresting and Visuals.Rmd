...
for (pos in positions) {
  ...
  # 2) Create a 20% validation split
...
for (pos in positions) {
  ...
  # 3) One‐hot encode (to avoid categorical‐level limits)
...
for (pos in positions) {
  ...
  # 3) One‐hot encode (fit on train only)
  dv         <- dummyVars(transfer ~ ., data = train_data, fullRank = TRUE)
  train_dmy  <- predict(dv, newdata = train_data)  %>% as.data.frame()
  train_dmy$transfer <- train_data$transfer
  ...
  # 5) Predict on TEST (20%) and build CM df
  ...
  # combine for plotting if you like, or plot separately
